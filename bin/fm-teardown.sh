...
# claim that proves neither refuses.
...
# when duplicate records do collide - that this same claim already resolved
# them: a claim naming THIS task rejected the duplicates as stale leftovers, and
# one naming another task is handled here. The scan cannot prove that THIS record
# is not the stale one, because the task that took the slot next may leave no
# record this scan can reach: its own worker may have exited and its record been
# cleaned up, or it may belong to a home this machine does not register. The
# claim closes that gap from the other side - it names the task that actually
# took the slot, and it is written under the same project lock that allocates it
# - so a claim naming another task is proof the slot was reassigned after this
# record was written.
#
